    ...
    def load_case(self, key):
        ...
        if 'open_data_file' in entry.keys():
            ...
            try:
                ...
                data = np.load(entry['data_file'][:-4] + ".npy")
            except:
                print(f"read data error: {entry['data_file'][:-4]}")
        ...
        if 'open_seg_file' in entry.keys():
            ...
            try:
                ...
                seg = np.load(entry['data_file'][:-4] + "_seg.npy")
            except:
                print(f"read seg error: {entry['data_file'][:-4]}")
            if self.keep_files_open:
