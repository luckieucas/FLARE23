...
class nnUNetDataLoader3DPartial(nnUNetDataLoaderBase):
    def __init__(self,
       ...
       # self.tumor_path = '/data/liupeng/nnUNet-master/DATASET/nnUNet_preprocessed/Dataset008_FLARE2023Filter/tumor_3d_midres'
        self.tumor_path = '/data/liupeng/nnUNet-master/DATASET/nnUNet_preprocessed/Dataset009_FLARE2023TumorCrop/nnUNetPlans_3d_verylowres'
    ...
    def _random_select_partial_type(self):
     ...
     #   index = different_type_cnt.index(790)
 #   #    print(different_type_cnt, index)
     #   different_type_cnt[index] = 400
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
         ...
         #   print(properties.keys())
            if '14' not in selected_partial_type:
              #  print(selected_partial_type)
 #               print('**********no tumor********** using cutmix')
#                mins = []
#                maxs = []
#                _,z0,h0,w0 = seg.shape
#                for k,v in properties['class_locations'].items():
#                    if len(v):
#                        tmp_min = np.min(v, axis = 0)
#                        tmp_max = np.max(v, axis = 0)
#                        mins.append(tmp_min[1:])
#                        maxs.append(tmp_max[1:])
#                if len(mins):
#                    mins = np.stack(mins, axis = 0)
#                    maxs = np.stack(maxs, axis = 0)
#                   # print(mins.shape)
#                   # print('mins:{},maxs:{}'.format(mins,maxs))
#                    minz,minh,minw = np.min(mins[:,0]),np.min(mins[:,1]),np.min(mins[:,2])
#                    maxz,maxh,maxw = np.max(maxs[:,0]),np.max(maxs[:,1]),np.max(maxs[:,2]) 
#                    minz, minh, minw = np.max([0,minz-16]), np.max([0,minh-32]), np.max([0,minw-32])
#                    maxz,maxh,maxw = np.min([z0,maxz+16]), np.min([h0, maxh+32]), np.min([w0, maxw+32])
#                    pseudo_bbox = [[minz,maxz],[minh,maxh],[minw,maxw]]
#                else:
#                    pseudo_bbox = [[0,z0],[0,h0],[0,w0]]
                ##
            #if 'organ_bbox' in properties:
 #               print(properties['class_locations'][14])
               # print('unsupervised data, key:{}'.format(i))
                pseudo_bbox = properties['organ_bbox_partial']
           #     print(pseudo_bbox)
