...
def make_out_dirs(dataset_id: int, task_name="FLARE"):
...
def copy_files(src_data_folder: Path, train_dir: Path, labels_dir: Path, test_dir: Path):
    ...
    patients_train = sorted([f for f in (src_data_folder / "Subtask1").iterdir() if f.is_dir()])
    patients_test = src_data_folder / "TestImage"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--input_folder",
        type=str,
        help="The downloaded FLARE2021 dataset dir. Should contain extracted 'training' and 'testing' folders.",
    )
    parser.add_argument(
        "-d", "--dataset_id", required=False, type=int, default=1, help="nnU-Net Dataset ID, default: 27"
    )
