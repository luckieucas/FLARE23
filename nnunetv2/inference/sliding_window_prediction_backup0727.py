...
def maybe_mirror_and_predict(network: nn.Module, x: torch.Tensor, mirror_axes: Tuple[int, ...] = None) \
    ...
    # try tensorRT
    # onnx_file = "/data1/liupeng/Flare2023/nnunet.onnx"
    # opt_shape_dict = {
    #     'input':[[1,1,32,192,192],
    #              [1,1,32,192,192],
    #              [1,1,32,192,192]]
    # }
...
def predict_sliding_window_return_logits(network: nn.Module,
    ...
    # print("export model for onnx")
    # dummy_input = torch.randn(1,1,32,192,192, requires_grad=True)
    # dummy_input = dummy_input.to(device)
    # torch.onnx.export(network, # model being run
    #                 dummy_input, # model input (or a tuple for multiple inputs)
    #                 "nnunet.onnx", # where to save the model
