...
from nnunetv2.postprocessing.remove_connected_components import remove_all_but_largest_component
from nnunetv2.utilities.label_handling.label_handling import convert_labelmap_to_one_hot as to_one_hot
import time
...
import SimpleITK as sitk
from batchgenerators.augmentations.utils import resize_segmentation
from nnunetv2.preprocessing.resampling.default_resampling import get_lowres_axis, get_do_separate_z, resample_data_or_seg
from batchgenerators.utilities.file_and_folder_operations import *
import time
import torch
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    shutil.copy(join(model, 'dataset.json'), join(output_folder, 'dataset.json'))
    shutil.copy(join(model, 'plans.json'), join(output_folder, 'plans.json'))
