        ...
        with torch.autocast(device.type, enabled=True):
            ...
            try:
                ...
                gaussian = gaussian.to(results_device)
                 ...
                 empty_cache(device)
            ####TODO filter empty z slices
            slicers_list = []
            hw_nums = []
            for sl_tmp in slicers:
                sl, num = sl_tmp
                slicers_list.append(sl)
                hw_nums.append(num)
            z_num = len(slicers_list) // (hw_nums[0]*hw_nums[0])
            if z_num > 4 and hw_nums[0] > 1:
                for z_idx in range(z_num):
                     # upper left
                     if hw_nums[0] == 2:
                         tmp_slicers = slicers_list[z_idx*4:(z_idx+1)*4]
                         workon = data[tmp_slicers[0]][None]
                         ...
                         predicted_logits[tmp_slicers[0]] += (prediction * gaussian if use_gaussian else prediction)
                         n_predictions[tmp_slicers[0][1:]] += (gaussian if use_gaussian else 1)
                         is_empty1 = torch.sum(torch.argmax(prediction, 0)>0)<100

                         workon = data[tmp_slicers[3]][None]
                         workon = workon.to(device, non_blocking=False)
                         prediction = maybe_mirror_and_predict(network, workon, mirror_axes)[0].to(results_device)
                         predicted_logits[tmp_slicers[3]] += (prediction * gaussian if use_gaussian else prediction)
                         n_predictions[tmp_slicers[3][1:]] += (gaussian if use_gaussian else 1)
                         is_empty2 = torch.sum(torch.argmax(prediction, 0)>0)<100
                         is_empty = is_empty1 and is_empty2
                   ...
                   ##     n_predictions[tmp_slicers[3][1:]] += (gaussian if use_gaussian else 1)
                    ...
                    # elif hw_nums[0] == 3:
                         ...
                         is_empty = torch.sum(torch.argmax(prediction, 0)>0)<100
                     ##is_empty = is_empty1 and is_empty2
                    #  if is_empty:
