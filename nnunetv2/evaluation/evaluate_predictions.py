...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    # load images
    print(f"prediction_file:{prediction_file}, reference_file:{reference_file}")
    seg_ref, seg_ref_dict = image_reader_writer.read_seg(reference_file)
    seg_pred, seg_pred_dict = image_reader_writer.read_seg(prediction_file)
    unique_label = np.unique(seg_ref)
    exclude_label = [label for label in range(0,15) if label not in unique_label]
   # print(f"prediction_file:{prediction_file}, reference_file:{reference_file}")  
    print(f"unique label{unique_label}, exclude_label:{exclude_label}")
    for label in exclude_label:
        seg_pred[seg_pred==label] = 0
...
#    print(f"pred file:{files_pred}")
#    print(f"ref file:{files_ref}")
    if not chill:
