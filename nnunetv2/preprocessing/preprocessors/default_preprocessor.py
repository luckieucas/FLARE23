...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        #if plans_manager.transpose_forward != [for i in range(0,len(plans_manager.transpose_forward))]:
        if plans_manager.transpose_forward != [i for i in range(0,len(plans_manager.transpose_forward))]:
        ...
        bbox = [[0,i] for i in shape_before_cropping]
        ...
        data = F.interpolate(torch.as_tensor(data[None,:,:,:,:]), size=tuple(new_shape), mode='trilinear')[0,...].numpy()
        #seg = np.zeros_like(data)
        #if has_seg:
        #seg = configuration_manager.resampling_fn_seg(seg, new_shape, original_spacing, target_spacing)
        seg = F.interpolate(torch.as_tensor(seg[None,:,:,:,:]).float(), size=tuple(new_shape), mode='nearest-exact')[0,...].numpy()
    ...
    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
                      plans_manager: PlansManager, configuration_manager: ConfigurationManager,
                      dataset_json: Union[dict, str]):
        if isfile(output_filename_truncated + '.npz') and isfile(output_filename_truncated + '.pkl'):
            print(output_filename_truncated,"is already truncated")
            return
        data, seg, properties = self.run_case(image_files, seg_file, plans_manager, configuration_manager, dataset_json)
        # print('dtypes', data.dtype, seg.dtype)
        if not isfile(output_filename_truncated + '.npz'):
            np.savez_compressed(output_filename_truncated + '.npz', data=data, seg=seg)
        if not isfile(output_filename_truncated + '.pkl'):
    ...
    def _normalize(self, data: np.ndarray, seg: np.ndarray, configuration_manager: ConfigurationManager,
        ...
        for c in range(data.shape[0]):
            ...
            #print(f"normalizer class: {normalizer_class}")
